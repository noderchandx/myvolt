...
function renderGrid(items){
  ...
  items.forEach(item=>{
    ...
    card.innerHTML = `
      ...
      ${item.type === "file" ? `<div class="dl-btn" data-id="${item.id}" title="${t("menu_download")}">⬇️</div>` : ""}
    ...
    card.addEventListener("click", (e)=>{
      if(e.target.closest(".menu-btn") || e.target.closest(".dropdown") || e.target.closest(".dl-btn")) return;
...
function handleItemAction(action, item){
  ...
  if(action === "download") triggerDownload(item.url, item.name);
